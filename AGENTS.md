...
- 每轮: 合并截屏(capture, 175×223) → 视图切片取牌面/分数 ROI → 卡住检测（与上轮截图比较，连续3次相同先尝试跳过，仍相同则报警；DEBUG 时从第2次起打印重复次数） → 先决条件检测(tile_matcher.check_precondition, TM_CCOEFF_NORMED，不满足时重试） → ORB BFMatcher(best_match, 无重试一次出结果） → OCR 分数检测(score_reader.check_number） → 自摸/跳过
