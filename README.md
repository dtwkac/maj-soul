...
4. **分数检测** — Tesseract OCR 读取分数，当前值低于最大值 2/3 时报警暂停
...
| `CONF_STRICT` | `{'9s':10, '1p':9, '9p':9, '9m':5}` | 各个模板的独立阈值 |
| `NUM_ALARM_RATIO` | 2/3 | 分数低于最大值 × 2/3 时报警 |
...
当 OCR 读取到 `cur/max` 格式且 `cur < max × 2/3` 时：
