...
2. **卡住检测** — 每轮比较当前牌面截图与上一轮，连续 3 次相同先尝试点击跳过，若仍相同则报警弹窗（先于先决条件，始终运行）
...
程序每轮对牌面 ROI 进行像素级比较（`np.array_equal`），连续 3 次完全相同时先执行一次"跳过"点击，若下一轮画面仍相同则触发报警。卡住检测位于先决条件检测之前，确保始终运行，不受条件控制流影响。  
...
### 卡住检测
- 每轮对牌面 ROI 进行像素级比较（`np.array_equal`），连续 3 次相同时先尝试点击跳过，下一轮仍相同则触发报警
