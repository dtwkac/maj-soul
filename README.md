...
6. **分数检测** — Tesseract OCR（灰度图 bilateralFilter 降噪保边，2 倍放大，Otsu 二值化，PSM 7）读取分数（格式 `abc/184`），`abc > 184` 或格式不符时视为无效重试；分数大幅下降时连续两次一致即接受；低于 `NUM_ALARM(105)` 时报警
...
- 输出格式为 `abc/184`，`abc > 184` 或格式不符时视为无效，走重试流程；首次识别或分数未大幅下降（`cur >= prev - 8`）时直接接受
